...
test('Models endpoint returns k2d6 and fallback models', async () => {
  ...
  globalThis.fetch = async (input: any) => {
    ...
    if (url.includes('/GetAvailableModels')) {
...
test('Chat Completions endpoint with k2d6 (thinking enabled)', async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (input: any) => {
    const url = typeof input === 'string' ? input : input.url;
    if (url.includes('/GetAvailableModels')) {
...
test('API Key protection', async () => {
  ...
  try {
    ...
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async () => new Response(JSON.stringify({ availableModels: [] }), { status: 200 });
...
test('Chat Completions endpoint - Non-streaming (stream: false)', async () => {
      ...
      const stream = new ReadableStream({
        start(c) {
          c.enqueue(encodeConnectRequest({
            op: 'set',
            mask: 'block.think',
            block: { think: { content: 'Thinking non-stream...' } }
          }));
          c.enqueue(encodeConnectRequest({
            op: 'set',
            mask: 'block.text',
            block: { text: { content: 'Hello non-stream' } }
          }));
          c.close();
        }
      });
  ...
  try {
    ...
    const body = await res.json();
    assert.strictEqual(body.object, 'chat.completion');
    assert.strictEqual(body.model, 'k2d6');
